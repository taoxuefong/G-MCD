...
from torchreid.losses import TripletLoss, CrossEntropyLoss#, Minimum_Camera_Discrepancy
...
class ImageTripletEngine(Engine):
    ...
    def train(
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            loss_x = torch.sum(torch.stack([self._compute_loss(self.criterion_x, logits, pids) for logits in outputs], dim=0))
            all_feat = torch.cat(features, dim=1)
            loss_t = self._compute_loss(self.criterion_t, all_feat, pids)
            #loss_t = self._compute_loss(self.criterion_t, features, features, pids, pids)
            #loss_x = self._compute_loss(self.criterion_x, outputs, outputs, pids, pids)
            #loss_mcds = self._compute_loss(self.criterion_mcd, outputs, camids, outputs, camids)
            
            loss = 0.1 * loss_t + self.weight_x * loss_x#+ loss_mcds*0.1
            loss.backward() 
            self.optimizer.step()
            ...
            #losses_mcd.update(loss_mcds.item(), pids.size(0))
